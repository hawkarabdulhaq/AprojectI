...
import sqlite3
from github_sync import push_db_to_github  # using updated GitHub sync function
        ...
        if submit_button:
            if not st.session_state.get("run_success", False):
                ...
                if updated_rows == 0:
                    ...
                    try:
                        # Call push_db_to_github only once to update the remote database
                ...
                # Clear the username state so the user must re-enter it on the next submission
                st.session_state["username_entered"] = False
                st.session_state["username"] = ""
            else:
