...
def show():
    ...
    if st.session_state.get("username_entered", False):
        ...
        st.markdown('<p style="color: white;">ðŸ“ Paste Your Code Here</p>', unsafe_allow_html=True)
        ...
        if st.session_state["run_success"]:
            st.markdown('<h3 style="color: white;">ðŸ“„ Captured Output</h3>', unsafe_allow_html=True)
            ...
            if st.session_state["map_object"]:
                st.markdown("### ðŸ—ºï¸ Map Output")
                st_folium(st.session_state["map_object"], width=1000, height=500)

            if st.session_state["dataframe_object"] is not None:
                st.markdown("### ðŸ“Š DataFrame Output")
