...
def show_login_create_account():
    ...
    # LOGIN TAB
    with tabs[0]:
        st.subheader("🔑 Login for Course 2")
        username = st.text_input("Username", key="loginx_username")
        password = st.text_input("Password", type="password", key="loginx_password")
        if st.button("Login", key="loginx_button"):
    ...
    # CREATE ACCOUNT TAB
    with tabs[1]:
        st.subheader("🆕 Create Account for Course 2")
        reg_fullname = st.text_input("Full Name", key="regx_fullname")
        reg_email = st.text_input("Email", key="regx_email")
        reg_phone = st.text_input("Mobile Number", key="regx_phone")
        reg_username = st.text_input("Username", key="regx_username")
        reg_password = st.text_input("Password", type="password", key="regx_password")
        if st.button("Register", key="registerx_button"):
    ...
    # FORGOT PASSWORD TAB
    with tabs[2]:
        st.subheader("🔒 Forgot Password for Course 2")
        forgot_email = st.text_input("Enter your registered email", key="forgotx_email")
        if st.button("Retrieve Password", key="retrievex_button"):
            if not forgot_email:
